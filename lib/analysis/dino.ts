 ...
 *
 * Deliberately shaped as a VisionResult (provider: "dino") rather than a
 * bespoke type: lib/analysis/aggregator.ts already knows how to weight,
 * consensus-check, and surface evidence for anything in `vision_results`
 * without caring which provider produced it. Reusing that shape means this
 * function is the *entire* integration — aggregator.ts didn't need a single
 * line changed to pick this signal up.
 *
 ...
 * missing DINO service never breaks or skews an analysis.
 */
export async function analyzeWithDino(imageBuffer: Buffer): Promise<VisionResult> {
  try {
    ...
    const data = (await response.json()) as { ai_probability?: unknown };
