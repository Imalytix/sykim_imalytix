...
export async function analyzeWithClaude(
  ...
  if (!apiKey) {
    return normalizeModelResult(null, "claude", modelName, { errorMessage: "ANTHROPIC_API_KEY is not configured." });
  }
  ...
  if (!text) {
    return normalizeModelResult(null, "claude", modelName, { errorMessage: "Claude returned an empty response." });
  }

  const parsed = extractJsonObject(text);
  return normalizeModelResult(parsed ?? text, "claude", modelName);
}
